...
//  display the base card rank if any foundation pile is empty
...
namespace CardGames.Canfield
{
	...
	/// <summary>
	/// This is a simple implementation of the solitare game Canfield.
	/// </summary>
	public class Canfield : System.Windows.Forms.Form
	{
		#region Constants
		...
		private void InitializeComponent()
		{
			...
			this.Text = "Canfield";
			this.MouseDown += new System.Windows.Forms.MouseEventHandler(this.Canfield_MouseDown);
			this.Closing += new System.ComponentModel.CancelEventHandler(this.Canfield_Closing);
			this.Paint += new System.Windows.Forms.PaintEventHandler(this.Canfield_Paint);
		...
		/// <param name="e"></param>
		private void Canfield_MouseDown(object sender, MouseEventArgs e)
		{
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void Canfield_Paint(object sender, PaintEventArgs e)
		{
			DrawCards();
		}
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void Canfield_Closing(object sender, CancelEventArgs e)
		{
			if (!finished)
			{
		...
		private void NewHand()
		{
			...
			theFoundation[0].Add(theDeck[13]);
			foundationStartRank = Card.RankFromCardIndex(theDeck[13]);
			cardsPlayed++;
			balance += 5;
			UpdateMoneyMeter();
			...
			{
				theTableau[count - 14].Add(theDeck[count]);
			}
		...
		private bool IsValidFoundationMove(int column, int movingCard)
		{
			...
			else if (theFoundation[column - 4].Count < 13)
			{
				...
				validMove = (currentRank == foundationStartRank);
			}
